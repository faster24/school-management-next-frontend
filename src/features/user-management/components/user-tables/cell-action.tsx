...
interface UserManagementCellActionProps {
  data: User;
}

export const UserManagementCellAction: React.FC<
  UserManagementCellActionProps
> = ({ data }) => {
  ...
  const onConfirm = async () => {
    const isSuccess = await deleteUser(data.id);
    if (isSuccess) {
      setOpen(false);
      router.refresh();
    }
